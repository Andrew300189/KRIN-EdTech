...
async function main() {
  ...
  // This technical author owns reference content and support defaults. It is
  // deliberately hidden from all learner-facing people lists and rankings.
  const seedInstructor = await prisma.user.upsert({ where: { email: "content@seed.krin.local" }, update: { role: "CONTENT_MANAGER", name: "Seed Content Manager", showInLeaderboard: false }, create: { id: "seed-content-manager", email: "content@seed.krin.local", username: "seed-content-manager", name: "Seed Content Manager", passwordHash: "seed-account-not-for-login", role: "CONTENT_MANAGER", emailVerified: true, showInLeaderboard: false } });
