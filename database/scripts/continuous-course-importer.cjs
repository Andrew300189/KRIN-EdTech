...
function createContinuousCourseImporter(config) {
  function makeExercises(fragment, skillSlug, serial) {
    ...
    return [
      ...
      task("spelling", "TEXT_INPUT", "text-input", config.spellingVariant ?? "ING_SPELLING", "Напишіть правильну форму без варіантів.", third.prompt, { acceptedAnswers: [third.answer], ignorePunctuation: true, example: third.correct, errorDetails: [{ incorrect: third.incorrect, correction: third.correct, explanation: third.rule }] }, third.answer, third.rule, config.spellingHint ?? "Перевірте форму to be та написання дієслова з -ing.", skillSlug),
      task("builder", "SENTENCE_ORDER", "sentence-builder", "SENTENCE_ORDER", "Розташуйте слова у правильному порядку.", `Побудуйте точне речення ${config.grammarName}.`, { options: tokens, preserveOrder: true, example: first.correct, errorDetails }, tokens, first.rule, config.builderHint ?? "Перевірте порядок підмета, to be та V-ing.", skillSlug),
  ...
  function fragmentBlocks(lessonPlan, moduleIndex, focus, fragmentIndex, offset) {
    ...
    return [
      ...
      },
      {
        ...
        content: { text: config.practiceDescription ?? "12 різнотипних вправ: вибір, зіставлення, пропуск, правопис, порядок слів, виправлення, трансформація, переклад, контекст і самостійна відповідь.", exerciseCount: PRACTICE_PER_FRAGMENT },
