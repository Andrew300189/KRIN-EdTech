...
import { CmsLessonDetailsEditor, CmsLessonEnhancements, InlineLessonTitleEditor } from "@/modules/cms/components/CmsLessonEditor";
...
export default async function CmsLessonPage({ params }: { params: Promise<{ lessonId: string }> }) {
  ...
  const lesson = await prisma.lesson.findUnique({ where: { id: lessonId }, include: { module: { include: { course: true } }, grammarSkills: { select: { grammarSkillId: true } }, _count: { select: { blocks: true } } } });
  if (!lesson) notFound();
  const [vocabulary, siblings, grammarSkills] = await Promise.all([
