...
function avatar(itemId: string) {
  return itemId === "avatar-fox" ? "🦊" : itemId === "avatar-owl" ? "🦉" : "✦";
}
...
export function ShopPage() {
  ...
  async function equip(item: ShopItem) {
    ...
    try {
      ...
      const payload = await response.json().catch(() => null) as { data?: { equippedTheme: string | null; equippedAvatar: string | null }; error?: string } | null;
      if (!response.ok || !payload?.data) throw new Error(payload?.error ?? text.error);
      if (item.kind === "theme") {
  ...
  return <section className={styles.page}>
    ...
    <div className={styles.grid}>{shop.items.map((item) => {
      ...
      return <article key={item.id} className={`${styles.item} ${equipped ? styles.itemEquipped : ""}`}>
        <div className={`${styles.preview} ${styles[`preview${item.kind[0].toUpperCase()}${item.kind.slice(1)}`]}`}>{item.kind === "avatar" ? avatar(item.id) : item.kind === "discount" ? "%" : item.id === "theme-aurora" ? "✦" : "☀"}</div>
