...
function AccessUpsell({ reason }: { reason: "AUTH_REQUIRED" | "PREMIUM_REQUIRED" | string }) {
  ...
  return <main className="mx-auto max-w-3xl px-6 py-12"><section className="rounded-2xl border border-amber-200 bg-amber-50 p-7"><h1 className="text-3xl font-bold text-amber-950">{title}</h1><p className="mt-3 text-amber-900">{message}</p><Link href={signedOut ? "/login" : backToCourse ? "/courses" : "/dashboard/billing"} className="mt-5 inline-flex rounded-lg bg-blue-700 px-4 py-2 font-semibold text-white hover:bg-blue-800">{signedOut ? "Sign in" : backToCourse ? "Back to course" : "View plans"}</Link></section></main>;
}
