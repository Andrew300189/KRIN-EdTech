...
export async function POST(request: NextRequest, { params }: { params: Promise<{ ticketId: string }> }) {
  ...
  if (!value.success) return NextResponse.json({ error: "Invalid support reply." }, { status: 400 });
  try { return NextResponse.json({ message: await addSupportMessage({ ticketId: (await params).ticketId, actorId: guard.user.id, actorIsAgent: true, ...value.data }) }, { status: 201 }); }
  catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to post reply." }, { status: 400 }); }
}
