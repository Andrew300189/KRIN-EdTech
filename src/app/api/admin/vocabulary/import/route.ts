...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error instanceof ZodError) return NextResponse.json({ error: "Invalid import data", issues: error.issues }, { status: 400 });
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to import vocabulary" }, { status: 400 });
  }
}
