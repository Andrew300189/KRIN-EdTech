...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    announcements: await prisma.systemAnnouncement.findMany({
...
export async function POST(request: NextRequest) {
  ...
  const announcement = await prisma.systemAnnouncement.create({
    data: {
      ...
      status: value.data.publishNow
        ? "PUBLISHED"
        : value.data.scheduledAt
          ? "SCHEDULED"
          : "DRAFT",
      scheduledAt: value.data.scheduledAt
        ? new Date(value.data.scheduledAt)
        : null,
      publishedAt: value.data.publishNow ? now : null,
      expiresAt: value.data.expiresAt ? new Date(value.data.expiresAt) : null,
    },
  });
