...
import { duplicateLessonBlock, removeLessonBlock, updateLessonBlock } from "@/modules/courses/services/content.service";
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ blockId: string }> }) {
  ...
  if (!guard.ok) return NextResponse.json({ error: guard.error }, { status: guard.status });
  try {
    return NextResponse.json({ data: await removeLessonBlock(guard.user.id, (await params).blockId) });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to remove lesson block." }, { status: 400 });
  }
}
