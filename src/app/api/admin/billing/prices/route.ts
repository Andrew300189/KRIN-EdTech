...
export async function POST(request: NextRequest) { const guard = await requireContentManager(request); if (!guard.ok) return NextResponse.json({ error: guard.error }, { status: guard.status }); const value = schema.safeParse(await request.json().catch(() => null)); if (!value.success) return NextResponse.json({ error: "Invalid price." }, { status: 400 }); try { return NextResponse.json({ price: await prisma.productPrice.create({ data: value.data }) }, { status: 201 }); } catch { return NextResponse.json({ error: "Provider price identifier already exists." }, { status: 409 }); } }
