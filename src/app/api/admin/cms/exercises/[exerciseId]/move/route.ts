...
import { moveCmsExercise } from "@/modules/cms/services/exercise-operations.service";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ exerciseId: string }> }) {
  ...
  try {
    const exercise = await moveCmsExercise(guard.user.id, (await params).exerciseId, parsed.data.targetLessonId);
    return NextResponse.json({ data: exercise });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to move exercise." }, { status: 400 });
  }
}
