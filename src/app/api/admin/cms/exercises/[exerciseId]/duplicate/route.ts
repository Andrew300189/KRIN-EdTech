...
import { duplicateCmsExercise } from "@/modules/cms/services/exercise-operations.service";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ exerciseId: string }> }) {
  ...
  try {
    const exercise = await duplicateCmsExercise(guard.user.id, (await params).exerciseId, parsed.data.targetLessonId);
    return NextResponse.json({ data: exercise }, { status: 201 });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to copy exercise." }, { status: 400 });
  }
}
