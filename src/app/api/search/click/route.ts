...
function parseContext(value: unknown): SearchContext {
  if (
    typeof value === "string" &&
    SEARCH_CONTEXTS.includes(value as SearchContext)
  ) {
...
function parseResultType(value: unknown): SearchResultType | null {
  ...
  return SEARCH_RESULT_TYPES.includes(value as SearchResultType)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rate = consumeRateLimit(
      user?.id
        ? `search-click:user:${user.id}`
        : `search-click:ip:${clientIp(request)}`,
      user ? 240 : 120,
      60_000,
    );
    if (!rate.allowed) {
    ...
    const position = Number.isFinite(positionRaw)
      ? Math.max(0, Math.floor(positionRaw))
      : -1;
