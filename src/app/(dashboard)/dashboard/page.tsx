...
const tourSteps: TourStep[] = [
  {
    id: "lessons",
    title: "Lessons",
    description: "This is where your daily lessons are organized by level and goals.",
    targetId: "tour-nav-lessons",
  },
  {
    id: "vocabulary",
    title: "Vocabulary",
    description: "Grow your active vocabulary and revise words with spaced practice.",
    targetId: "tour-nav-vocabulary",
  },
  {
    id: "profile",
    title: "Profile",
    description: "Update your goals, language settings, and intensity any time.",
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl bg-gradient-to-br from-primary/15 via-white to-secondary/10 border border-primary/20 p-6">
        <p className="text-sm uppercase tracking-wide text-primary/80">Welcome aboard</p>
        <h1 className="text-3xl font-bold mt-2">Hi, {userName}. Let us start your first learning win.</h1>
        <p className="mt-3 text-gray-700 max-w-2xl">
          Your path is ready. Start your first lesson or take a quick level test to personalize recommendations.
        </p>
      ...
      <section className="grid md:grid-cols-2 gap-4">
        <article className="rounded-xl bg-white border border-gray-200 p-5 shadow-sm">
          ...
          <p className="text-sm text-gray-600 mt-1">Your first day goal: 0/{dailyGoalMinutes} minutes</p>
          <div className="mt-4 h-3 rounded-full bg-gray-200 overflow-hidden">
            <div className="h-full w-0 bg-primary" />
          </div>
          <p className="mt-3 text-sm text-primary font-medium">Complete one lesson to fill your first progress bar.</p>
        </article>
        ...
        <article className="rounded-xl bg-white border border-amber-200 p-5 shadow-sm">
          ...
          <p className="text-sm text-gray-700 mt-2">You unlocked {bonusPoints} KRIN points and starter flashcards pack.</p>
      ...
      {profile.takePlacementTest ? (
        <section className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-blue-900">
          Quick test requested during onboarding. Open the AI tutor test flow to refine your level.
        </section>
      ) : null}
      ...
      {showTour && currentStep ? (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-end md:items-center justify-center p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <p className="text-xs uppercase tracking-wide text-gray-500">Guided tour {tourIndex + 1}/{tourSteps.length}</p>
            <h3 className="text-xl font-bold mt-2">{currentStep.title}</h3>
            <p className="mt-3 text-gray-700">{currentStep.description}</p>
            <div className="mt-6 flex justify-between">
              <button className="btn btn-secondary" onClick={() => setShowTour(false)} type="button">
                Skip
              </button>
              <button className="btn btn-primary" onClick={nextTourStep} type="button">
