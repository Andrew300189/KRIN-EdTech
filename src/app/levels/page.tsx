  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-12">
      ...
      <section className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3" aria-label="CEFR levels">
        {levels.map((level) => <Link key={level.id} href={`/levels/${level.code.toLowerCase()}`} className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-blue-300 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"><p className="text-3xl font-bold text-blue-700">{level.code}</p><h2 className="mt-2 text-xl font-bold text-slate-900">{level.title}</h2><p className="mt-2 text-sm text-slate-600">{level.description}</p><p className="mt-5 text-sm font-semibold text-blue-700">{level._count.courses} courses <span aria-hidden="true">→</span></p></Link>)}
      </section>
    </main>
  );
}
