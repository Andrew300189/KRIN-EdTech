      ...
      <div className={styles.courseMain}>
        ...
        <section className={styles.courseReviews} aria-labelledby="course-reviews-title">
          <div className={styles.courseReviewsHeading}>
            <div><p className={styles.outcomesEyebrow}>Learner feedback</p><h2 id="course-reviews-title">Course reviews</h2></div>
            {publicReviews.total ? <p className={styles.courseRatingSummary}><span aria-hidden="true">★</span> {publicReviews.averageRating?.toFixed(1)}/7 <small>from {publicReviews.total} {publicReviews.total === 1 ? "review" : "reviews"}</small></p> : null}
          </div>
          {publicReviews.reviews.length ? <div className={styles.courseReviewList}>{publicReviews.reviews.map((review) => <article key={`${review.reviewerName}-${review.updatedAt.toISOString()}`} className={styles.publicCourseReview}>
            <div className={styles.publicCourseReviewMeta}><div><strong>{review.reviewerName}</strong><span className={styles.publicCourseReviewStars} aria-label={`${review.rating} of 7 stars`}>{stars(review.rating)}</span></div><time dateTime={review.updatedAt.toISOString()}>{new Intl.DateTimeFormat(locale?.startsWith("ru") ? "ru-RU" : "en", { month: "short", year: "numeric" }).format(review.updatedAt)}</time></div>
            <p>{review.comment}</p>
