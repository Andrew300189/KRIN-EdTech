...
export function VocabularyTrainingPlayer({ sessionId, compact = false, onCompleted }: { sessionId: string; compact?: boolean; onCompleted?: () => void }) {
  ...
  async function submit(value?: string) {
    ...
    try {
      const response = await fetch(`/api/profile/vocabulary/session-items/${item.id}/answer`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ submittedAnswer: value ?? answer, responseTimeSeconds: Math.round((Date.now() - startedAt.current) / 1000), confidence }) });
