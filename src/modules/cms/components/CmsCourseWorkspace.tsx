...
export function CmsCourseWorkspace({ initialCourses, levels, categories }: {
  ...
  return <section className="space-y-6">
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4"><label className="block max-w-sm text-sm font-medium text-slate-700">Course format<select value={courseType} onChange={(event) => setFilter("type", event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2"><option value="ALL">All formats</option><option value="STANDARD">Standard</option><option value="INTENSIVE">Intensive</option><option value="EXAM_PREP">Exam preparation / tests</option><option value="PROFESSIONAL">Professional English</option><option value="SPECIALIZATION">Specialization</option><option value="SKILL">Skill course</option></select></label><p className="mt-2 text-xs text-slate-500">Review the course types that drive the public Professional English and English tests collections.</p></div>
