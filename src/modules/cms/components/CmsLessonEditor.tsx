...
const curriculumRoles = ["STANDARD", "OVERVIEW", "DEEP_DIVE", "PRACTICE", "REVIEW", "FINAL"] as const;
...
type Lesson = {
  ...
  isFree: boolean;
  order: number;
  grammarSkills?: Array<{ grammarSkillId: string }>;
};

type AvailableLesson = { id: string; title: string; order: number };
type AvailableGrammarSkill = { id: string; title: string; slug: string };
...
export function CmsLessonDetailsEditor({ lesson, availableLessons, availableGrammarSkills }: { lesson: Lesson; availableLessons: AvailableLesson[]; availableGrammarSkills: AvailableGrammarSkill[] }) {
  const router = useRouter();
    ...
    startTransition(async () => {
      try {
        await request(`/api/admin/lessons/${lesson.id}`, "PATCH", {
          ...
          type: form.get("type"),
          curriculumRole: form.get("curriculumRole"),
          estimatedDuration: Number(form.get("estimatedDuration") || 0),
          minimumCompletionScore: Number(form.get("minimumCompletionScore") || 0),
          ...
          grammarSkillIds: form.getAll("grammarSkillIds"),
        });
