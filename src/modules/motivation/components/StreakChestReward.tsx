    ...
    <section className={styles.card} style={chestStyle} aria-live="polite">
      ...
      {!reward ? <>
        ...
        {reward.questBook ? <section className={styles.questBook}>
          ...
          <div><strong>{text.bookFound}</strong><p>{text.bookDescription.replace("{coins}", String(reward.questBook.unlockCost)).replace("{words}", String(reward.questBook.target))}</p><Link href="/student/achievements#quest-books" onClick={onDismiss}>{text.openBook}</Link></div>
        </section> : null}
