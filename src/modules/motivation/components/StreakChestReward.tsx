...
export function StreakChestReward({ milestone, onDismiss }: { milestone: number | null; onDismiss: () => void }) {
  ...
  const kindLabel = kind === "restore" ? text.restore : kind === "coin" ? text.coin : kind === "xpCoin" ? text.xpCoin : kind === "hint" ? text.hint : kind === "translation" ? text.translation : text.regular;
    ...
    <section className={styles.card} style={chestStyle} aria-live="polite">
      ...
      {!reward ? <>
        ...
        <section className={`${styles.reward} ${styles[kind!] ?? ""} ${flower?.rarity === "LEGENDARY" ? styles.legendaryReward : ""}`}>
          <span aria-hidden="true">{flower?.icon ?? (kind === "restore" ? "❄" : kind === "coin" ? "●" : kind === "xpCoin" ? "◈" : kind === "hint" ? "☀" : kind === "translation" ? "✧" : "✦")}</span>
          <div><strong>{flowerName ? `${flowerName} · ${text.rarities[flower!.rarity]}` : kindLabel}</strong><p>+{reward.experience} XP{reward.coins ? ` · +${reward.coins} KRIN Coin` : ""}{reward.xpCoins ? ` · +${reward.xpCoins.toFixed(2)} XP Coins` : ""}{reward.hintCredits ? ` · +${reward.hintCredits} ${text.hint}` : ""}{reward.translationCredits ? ` · +${reward.translationCredits} ${text.translation}` : ""}{reward.streakRestore ? ` · +${reward.streakRestore} ${text.restore}` : ""}</p></div>
        </section>
        {reward.streakRestore ? <p className={styles.restoreNote}>{text.restoreNote}</p> : null}
