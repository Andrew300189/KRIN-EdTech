    ...
    <section className={styles.card} style={chestStyle} aria-live="polite">
      ...
      {!reward ? <>
        ...
        <section className={`${styles.reward} ${styles[kind!] ?? ""} ${flower?.rarity === "LEGENDARY" ? styles.legendaryReward : ""}`}>
          <span aria-hidden="true">{flower?.icon ?? (kind === "coin" ? "●" : kind === "xpCoin" ? "◈" : kind === "hint" ? "☀" : kind === "translation" ? "✧" : "✦")}</span>
          <div><strong>{flowerName ? `${flowerName} · ${text.rarities[flower!.rarity]}` : kindLabel}</strong><p>+{reward.experience} XP{reward.coins ? ` · +${reward.coins} KRIN Coin` : ""}{reward.xpCoins ? ` · +${reward.xpCoins.toFixed(2)} XP Coins` : ""}{reward.hintCredits ? ` · +${reward.hintCredits} ${text.hint}` : ""}{reward.translationCredits ? ` · +${reward.translationCredits} ${text.translation}` : ""}</p></div>
        </section>
