...
export function AdminSearchAnalyticsTools({ defaultDays = 30 }: { defaultDays?: number }) {
  ...
  const runCleanup = async () => {
    ...
    if (!Number.isFinite(parsedRetention) || parsedRetention < 30 || parsedRetention > 1095) {
      setMessage("Retention must be between 30 and 1095 days.");
      return;
    }

    if (!dryRun) {
      const approved = window.confirm("This will permanently delete old search analytics data. Continue?");
      if (!approved) return;
    }
  ...
  return (
    <section className="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-4">
      <h3 className="text-sm font-semibold text-slate-900">Search analytics tools</h3>
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-2">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          <p className="text-sm font-medium text-slate-800">Retention cleanup</p>
          ...
          >
            {isSubmitting ? "Running..." : dryRun ? "Run dry cleanup" : "Run cleanup"}
          </button>
        </div>
      </div>
