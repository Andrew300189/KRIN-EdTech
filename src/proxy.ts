...
function isPremiumCoursePath(pathname: string) {
  ...
  return (
    first === "courses" &&
    Boolean(level) &&
    PREMIUM_COURSE_LEVELS.has(level.toLowerCase())
  );
}
...
async function verifySignedCookie(value: string | undefined) {
  ...
  const secret =
    ...
    "krin-dev-insecure-session-secret-change-me";
...
export async function proxy(request: NextRequest) {
  ...
  if (pathname === "/cms" || pathname.startsWith("/cms/")) {
    ...
    if (!email || !isPlatformOwner(email)) {
      return NextResponse.redirect(
        new URL(
          normalizeRole(session.role) === "teacher" ? "/teacher" : "/student",
          request.url,
        ),
      );
    }
  ...
  if (
    ...
    session &&
    isCmsRoute(pathname) &&
    !hasCmsAccess(session.email, session.role)
  ) {
    return NextResponse.redirect(
      new URL(
        normalizeRole(session.role) === "teacher" ? "/teacher" : "/student",
  ...
  if (isPremiumCoursePath(pathname)) {
    ...
    if (
      !session ||
      !session.userId ||
      !(await hasPremiumAccessCookie(request, session.userId))
    ) {
