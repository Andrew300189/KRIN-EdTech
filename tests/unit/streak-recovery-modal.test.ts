...
describe("lost streak recovery prompt", () => {
  ...
  it("opens even when an old session flag exists and stays dismissed during refresh", async () => {
    const view = render(createElement(LocaleProvider, null, createElement(DailyStreakHeaderStatus, { showBadge: false })));
    ...
    fireEvent.click(screen.getByRole("button", { name: /continue lesson|продовжити урок|продолжить урок/i }));
