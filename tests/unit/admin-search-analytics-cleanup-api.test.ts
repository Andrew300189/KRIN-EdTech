...
describe("api admin search analytics cleanup route", () => {
  beforeEach(() => {
    jest.resetAllMocks();
    mockedCleanup.mockResolvedValue({ retentionDays: 180, dryRun: true, deletedHistoryRows: 0, deletedMetricRows: 0 });
  });

  it("returns 403 when guard fails", async () => {
    mockedGuard.mockResolvedValue({ ok: false, status: 403, error: "Forbidden" } as never);

    const response = await POST(new NextRequest("http://localhost:3000/api/admin/analytics/search/cleanup", { method: "POST", body: "{}" }));
  ...
  it("validates payload", async () => {
    ...
    const response = await POST(new NextRequest("http://localhost:3000/api/admin/analytics/search/cleanup", {
      method: "POST",
  ...
  it("runs cleanup", async () => {
    ...
    expect(mockedCleanup).toHaveBeenCalledWith({ retentionDays: 365, dryRun: true });
  });
});
